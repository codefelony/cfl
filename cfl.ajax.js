// ==UserScript==
// @name          CFL - AJAX plugin
// @namespace     https://greasyfork.org/en/users/10166-moriarty
// @description   An AJAX plugin for CFL
// @include       *
// @version       1.1.0
// @author        Moriarty
// @require       https://raw.github.com/codefelony/cfl/master/versions/cfl-1.3.1.js
...
1.1.0 (3/28/2014)
    - added CFL.ajaxClear() to clear all current and pending requests
...
1.0.2 (10/6/2013)
    ...
    - added a .clear() method
        it may be called on any ajax instance like CFL.ajax(...).clear()
        it can even simply be called as CFL.ajax().clear()
...
(function (undefined) {
    ...
    function init(url, settings) {
        ...
        if (typeof url === 'string') {
            urls.push(url);
        } else if (CFL.typeOf(url) === 'array') {
            urls = urls.concat(url);
        }

        if (CFL.typeOf(settings) === 'object') {
            for (key in settings) {
                value = settings[key];
                ...
                switch (key) {
                    ...
                    case 'data': {
                        if (typeof value === 'string') {
                            realSettings[key] = value;
                        } else if (CFL.typeOf(value) === 'object') {
